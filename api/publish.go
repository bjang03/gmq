...
func Publish(ctx context.Context, req *PublishReq) (res interface{}, err error) {
	proxy := core.GetGmq(req.MqName)
	if proxy == nil {
		return nil, fmt.Errorf("[%s] proxy not found", req.MqName)
	}
	...
	switch req.MqName {
	case "nats":
		err = proxy.GmqPublish(ctx, &mq.NatsPubMessage{PubMessage: baseMsg, Durable: req.Durable})
	case "rabbitmq":
		err = proxy.GmqPublish(ctx, &mq.RabbitMQPubMessage{PubMessage: baseMsg, Durable: req.Durable})
	case "redis":
		err = proxy.GmqPublish(ctx, &mq.RedisPubMessage{PubMessage: baseMsg})
	default:
...
func PublishDelay(ctx context.Context, req *PublishDelayReq) (res interface{}, err error) {
	...
	switch req.MqName {
	case "nats":
		err = proxy.GmqPublishDelay(ctx, &mq.NatsPubDelayMessage{PubDelayMessage: baseMsg, Durable: req.Durable})
	case "rabbitmq":
		err = proxy.GmqPublishDelay(ctx, &mq.RabbitMQPubDelayMessage{PubDelayMessage: baseMsg, Durable: req.Durable})
	case "redis":
